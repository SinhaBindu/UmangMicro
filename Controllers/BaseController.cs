...
namespace KGBV_JH.Controllers
{
    public class BaseController : Controller
    {
        ...
        {
            AddAlert(UmangMicro.Models.AlertStyles.Information, message, dismissable);
        }
        ...
        public JsonResult GetDistrictList(string id)
        {
            ...
            if (!string.IsNullOrEmpty(id))
            {
                ...
                var block = from b in db.Dist_Mast.Where(x => x.StateId == _stateId)
                            select new { b.DistName, b.ID };
                list = new SelectList(block.Select(m => new { Text = m.DistName, Value = m.ID }).OrderBy(m => m.Text), "Value", "Text").ToList();
        ...
        public JsonResult GetBlockList(string id)
        {
            ...
            if (!string.IsNullOrEmpty(id))
            {
                ...
                var block = from b in db.Block_Mast.Where(x => x.DistId_fk == _blockId)
                            select new { b.BlockName,b.ID };
                list = new SelectList(block.Select(m => new { Text = m.BlockName, Value = m.ID }).OrderBy(m => m.Text), "Value", "Text").ToList();
                // var data = db.m_block.Where(x => x.DistrictId_Fk == district.DistId_Pk).ToList().OrderBy(o => o.Block).Select(b => new SelectListItem { Value = b.Block, Text = b.Block.ToString() });
                return Json(list, JsonRequestBehavior.AllowGet);
            }
            else
